...
# -*- coding: utf-8 -*-
"""
XOAD Discord Bot - 最小サンプル（SQLite/discord.py）
機能:
...
import os, sqlite3, asyncio, math, datetime as dt
...
async def report_cmd(interaction: discord.Interaction, rtype: str, details: str):
    ...
    await interaction.response.send_message(f"申請を受け付けました（ID: **{rid}** / type: `{rtype}`）。運営の承認をお待ちください。", ephemeral=True)
...
if __name__ == "__main__":
    if not TOKEN:
        raise RuntimeError("環境変数 BOT_TOKEN が未設定です(.envを用意してください)")
    bot.run(TOKEN)
