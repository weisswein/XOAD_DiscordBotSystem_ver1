...
async def on_ready():
    ...
    try:
        if GUILD_ID:
            ...
            print(f"Syncing commands for guild: {guild}")
            # tree.clear_commands(guild=guild)
            await tree.sync(guild=guild)
            print("✅ ギルドコマンドもクリアして再同期しました")
        else:
            await tree.sync()
...
class ReportActionView(discord.ui.View):
    ...
    async def disable_all(self, interaction: discord.Interaction, label: str):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not row:
            ...
            await self.disable_all(interaction, "承認")
            return
        ...
        try:
            ...
            await member.send(
                f"[申請承認] あなたの申請(ID {self.report_id}) は承認され、{xp}XP が付与されました。"
            )
        except Exception:
            pass
        await self.disable_all(interaction, "承認")
...
class RejectReasonModal(discord.ui.Modal, title="申請否認理由の入力"):
    reason = discord.ui.TextInput(label="否認理由", style=discord.TextStyle.paragraph, required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        row = decide_report(self.report_view.report_id, interaction.user.id, False)
        if not row:
            await interaction.response.send_message("否認できません（既に処理済みかID不正）。", ephemeral=True)
            await self.report_view.disable_all(interaction, "否認")
            return
        await interaction.response.send_message(f"ID {self.report_view.report_id} を否認しました。", ephemeral=True)
        # 否認DM送信
        try:
            member = await interaction.guild.fetch_member(self.report_view.user_id)
            await member.send(
                f"[申請否認] あなたの申請(ID {self.report_view.report_id}) は否認されました。\n理由: {self.reason.value}"
            )
        except Exception:
            pass
        await self.report_view.disable_all(interaction, "否認")
...
@tree.command(
    name="report_xoad",
    description="XOAD広告協力申請（協力内容とSNSリンクを入力）",
    guild=discord.Object(id=GUILD_ID) if GUILD_ID else None
)
@app_commands.describe(
    content="協力した広告の内容（協力元の名前を含む）",
    sns_link="投稿したSNSのリンク"
)
async def report_xoad(
    interaction: discord.Interaction,
    content: str,
    sns_link: str
):
    ...
    rid = create_report(user.id, "xoad_sns", details)
    # Embedで通知
    embed = discord.Embed(
        title="XOAD広告協力申請",
        color=discord.Color.green(),
        description=f"申請ID: {rid}"
    )
    embed.add_field(name="協力内容", value=content, inline=False)
    embed.add_field(name="SNSリンク", value=sns_link, inline=False)
    embed.add_field(name="申請者", value=user.mention, inline=True)
    view = ReportActionView(rid, user.id, "xoad_sns", details)
    # 通知チャンネルに送信
    channel = interaction.guild.get_channel(REPORT_NOTIFY_CHANNEL_ID)
    if channel:
        await channel.send(embed=embed, view=view)
    await interaction.response.send_message("申請を受け付けました。", ephemeral=True)

# ...既存のreport_cmd（/report）コマンドは不要なら削除、または用途別に残してもOK...
